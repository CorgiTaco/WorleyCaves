...
public class WorldCarverWorley extends WorldCarver<ProbabilityConfig> {
	...
	private static final Direction[] FLUID_CHECK_DIRECTIONS = new Direction[]{Direction.UP, Direction.EAST, Direction.WEST, Direction.NORTH, Direction.SOUTH};
    ...
    public static final Map<Block, BlockStateProvider> GRAVITY_BLOCK_NEIGHBORING_FLUIDS_REPLACEMENTS = Util.make(new Object2ObjectOpenHashMap<>(), map -> {
        map.put(Blocks.GRAVEL, new SimpleBlockStateProvider(Blocks.STONE.defaultBlockState()));
        map.put(Blocks.SAND, new SimpleBlockStateProvider(Blocks.SANDSTONE.defaultBlockState()));
        map.put(Blocks.RED_SAND, new SimpleBlockStateProvider(Blocks.RED_SANDSTONE.defaultBlockState()));
    });
    ...
    public boolean carve(IChunk chunkIn, Function<BlockPos, Biome> getBiomeFunction, Random rand, int seaLevel, int chunkX, int chunkZ, int chunkXOffset, int chunkZOffset, BitSet carvingMask, ProbabilityConfig config) {
        final CarverWorldAccess carverWorldAccess = CarverWorldAccess.peek();
        if (carverWorldAccess == null) {
            return false; // We're not in the air stage.
        }


        ServerWorld world = carverWorldAccess.getWorld();

        if (world == null) {
            Main.LOGGER.error("ERROR: Unable to retrieve world from CarvingContext!");
            return false;
        }

        if (!WorleyConfig.validDimensions.contains(world.dimension().location().toString())) {
            return false;
        }
        ...
        this.carveWorleyCaves(chunkIn, carverWorldAccess.getMask(GenerationStage.Carving.AIR), rand, getBiomeFunction, seaLevel, chunkX, chunkZ);
        CarverWorldAccess.pop();
    ...
    protected void carveWorleyCaves(IChunk chunk, BitSet carvingMask, Random random, Function<BlockPos, Biome> getBiomeFunction, int seaLevel, int chunkX, int chunkZ) {
                            ...
                            for (int subz = 0; subz < 4; subz++) {
                                ...
                                if (noiseVal > adjustedNoiseCutoff) {
                                    ...
                                    if (!isFluidBlock(aboveBlock) || localY <= lavaDepth) {
                                        ...
                                        digBlock(chunk, carvingMask,  random, currentBiome, localPos, foundTopBlock, currentBlock, aboveBlock);
                                    }
                                }
...
//		Biome biome = chunk.getBiome(blockPos);
     ...
     * @param blockPos Local chunk block pos(0-15 x/z)
     * @param foundTop True if we've encountered the biome's top block. Ideally if
     *                 we've broken the surface.
     */
    private void digBlock(IChunk chunk, BitSet carvingMask, Random random, Biome biome, BlockPos blockPos, boolean foundTop, BlockState state, BlockState blockStateUp) {
        ...
        if (this.canReplaceBlock(state, blockStateUp) || stateBlock == top.getBlock() || stateBlock == filler.getBlock()) {
            if (blockPos.getY() <= lavaDepth) {
                chunk.setBlockState(blockPos, lavaBlock, false);
            } else {

                boolean carveAir = true;
                for (Direction waterCheckDirection : FLUID_CHECK_DIRECTIONS) {
                    final BlockPos relative = blockPos.relative(waterCheckDirection);
                    if (relative.getX() <= 15 && relative.getZ() <= 15 && relative.getX() >= 0 && relative.getZ() >= 0) {
                        final BlockState blockState = chunk.getBlockState(relative);
                        if (!blockState.getFluidState().isEmpty() || GRAVITY_BLOCK_NEIGHBORING_FLUIDS_REPLACEMENTS.containsKey(blockState.getBlock())) {
                            carveAir = false;
                            chunk.setBlockState(blockPos, GRAVITY_BLOCK_NEIGHBORING_FLUIDS_REPLACEMENTS.getOrDefault(blockState.getBlock(), new SimpleBlockStateProvider(Blocks.STONE.defaultBlockState())).getState(random, blockPos), false);
                        }
                    }
                }
                if (carveAir) {
